...
class User extends Koneksi
{
            ...
            if ($stmt->rowCount()>0) {
                ...
                while ($data =$stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo '<tr>
                        ...
                        <td>'.$data['level'].'</td>
                        <td>
                            <a class="btn btn-success btn-sm glyphicon glyphicon-edit" href="?kanal=ms_user&act=edit&id='.$data['id_user'].'"></a>
    ...
    public function edituser($iduser,$nama,$kontak,$email,$username,$password,$level)
    {
        try{
            if (empty($password)) {
                $query = "UPDATE user SET nama=:nama,kontak=:kontak,email=:email,username=:username,level=:level
